...
function SetbuyMode(mode){
    for (let i = 0; i < nbUpgrades; i++){
        ...
        const milestone_bar_prediction = document.getElementById(`milestone_bar_prediction_${i+1}`);
        milestone_bar_prediction.style.width = (((xp_milestone[i]+n)/maxxp_milestone_bar[i])*100) + "%";
        if((xp_milestone[i]+n >= maxxp_milestone_bar[i])){
            milestone_bar_prediction.style.width = 100 + "%";
          }
    }
}
function SwitchBuyMode(){ //Changement du texte quand on appuie sur le bouton BuyMode
    mode++;
    if(mode >=4){
      mode=0;
    }
    ...
    for(let i = 0; i < nbUpgrades;i++){
        ...
        upbox.setAttribute("onclick", "buyUpgrades(" + i + ", " + mode + ")");
...
addEventListener("load", function(){
    ...
    for(let i = 0; i < nbUpgrades; i++){
        ...
        milestone_bar.style.width = ((xp_milestone[i]/maxxp_milestone_bar[i])*100) + "%";
        if(maxxp_milestone[i] == 0){
            maxxp_milestone[i] =10;
          }
          const upbox = document.getElementById(`upbox${i+1}`);
          upbox.classList.add("shadow_" + i + "_" + milestone_number[i]);
    }
